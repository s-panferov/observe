...
impl Reaction {
    fn new(design: Box<dyn ReactionRun>) -> Self {
...
trait ReactionRun {
    fn run_reaction(&mut self, ctx: &mut EvalContext);
}

struct Autorun {
    cb: Box<dyn Fn(&mut EvalContext)>,
}
...
struct Expression<T: Eq> {
    ...
    eff: Box<dyn Fn(&mut T, &mut EvalContext)>,
    cached: Option<T>,
}
...
impl ReactionBody {
    pub fn new(calc: Box<dyn ReactionRun>) -> Self {
        ReactionBody {
            calc: calc,
